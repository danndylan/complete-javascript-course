...
// accumulator -> snowball (consolidates everythinb)
...
const checkDogs = function (dogsJulia, dogsKate) {
  ...
  dogs.forEach((age, i) => {
    ...
    console.log(`Dog ${i + 1} is ${isAdult}, and is ${age} years old.`);
  });
};
...
const calcAverageHumanAge = function (dogAges) {
  const dogHumanAge = dogAges.map(age => (age <= 2 ? 2 * age : 16 + age * 4));
  const filteredAges = dogHumanAge.filter(age => age >= 18);
  const aveAge = filteredAges.reduce(
...
const refactoredCalcAverageHumanAge = dogAges =>
  dogAges
    .map(age => (age <= 2 ? 2 * age : 16 + age * 4))
    .filter(age => age >= 18)
    .reduce((total, age, i, arr) => (total += age / arr.length), 0);
console.log(
  refactoredCalcAverageHumanAge(testData1),
  refactoredCalcAverageHumanAge(testData2)
);
...
const numDeposits1000 = accounts
  .flatMap(acc => acc.movements)
  .reduce((len, mov) => (mov >= 1000 ? ++len : len), 0);
console.log(numDeposits1000);
...
1. Loop over the array containing dog objects, and for each dog, calculate
 ...
 NOT create a new array, simply loop over the array. Forumla: recommendedFood 
 = weight ** 0.75 * 28. (The result is in grams of food, and the weight needs 
 to be in kg)
...
// portion in an ascending order (keep in mind that the portions are inside the
// array's objects)
const sortedDogsFood = dogs
  .slice()
  .sort((a, b) => a.recommendedFood - b.recommendedFood);
console.log(sortedDogsFood);
